...
/*------------------------------------------------------------------------------*/
    $(".quantity-button2").on("click", function () {
        ...
        if ($(this).text() == "+") {
            if (
                document.getElementById("warning-message").hasAttribute("style")
            ) {
                document
...
/*------------------------------------------------------------------------------*/
    ...
    $(".menu-vertical > li.parent-plus > a, .menu-vertical > li.parent > a").on(
        "click",
        function (e) {
            ...
            if ($(this).hasClass("open")) {
                $(this)
                    .removeClass("open")
                    .next(".plus-menu, .megamenu")
                    .slideUp(300);
                return;
            }
            $(this)
                .parents(".menu-vertical")
                .find("> li.parent-plus > a, .menu-vertical > li.parent > a")
                .removeClass("open");
            $(this)
                .parents(".menu-vertical")
                .find(".plus-menu, .megamenu")
                .not(":hidden")
                .slideUp(300);
            $(this)
                .addClass("open")
...
function changeBillingType() {
    var select_status = $("#billingType").val();
    if (select_status == "Person") {
        $("#person").show();
        $("#company").hide();
    } else if (select_status == "Company") {
        $("#person").hide();
        $("#company").show();
    }
}
...
window.onload = function () {
    ...
    var cardnumber_mask = new IMask(cardnumber, {
        mask: [
            ...
            {
                mask: "0000 0000 0000 0000",
                regex: "^(?:9792|65\\d{0,2}|64[4-9]\\d?)\\d{0,12}",
                cardtype: "troy",
            },
    ...
    let troy = `<svg id="Page-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 102 48">
    <g id="troy" fill="none" fill-rule="evenodd">
    ...
    let troy_single = `<svg id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" width="102" height="48" viewBox="0 0 102 48">
    ...
    cardnumber_mask.on("accept", function () {
        ...
        switch (cardnumber_mask.masked.currentMask.cardtype) {
            ...
            case "mastercard":
                ...
                break;
            case "troy":
                ccicon.innerHTML = troy;
                ccsingle.innerHTML = troy_single;
                swapColor("cyan");
                break;
            case "unionpay":
