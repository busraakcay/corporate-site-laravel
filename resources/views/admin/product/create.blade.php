...
<div class="card card-custom card-sticky" id="kt_page_sticky_card">
    ...
    <form action="{{ route('admin.products.store') }}" class="form" id="kt_form" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card-body">
            <div class="mx-2">
                <div class="form-group">
                    <label for="code">Ürün Kodu <span class="text-danger">*</span></label>
                    <input type="text" id="code" name="code" value="{{ old('code')}}" class="form-control" placeholder="Ürün kodunu giriniz" />
                </div>
                <div class="form-group">
                    ...
                    <input type="text" id="size" name="size" value="{{ old('size')}}" class="form-control" placeholder="Ürün asortisini giriniz: 40-1,41-2,42-2" />
                </div>
            </div>
                    ...
                    <div class="tab-pane fade {{ $language['code'] == 'tr' ? 'active show' : '' }}" id="kt_tab_pane_{{$language["code"]}}_2" role="tabpanel">
                        <div class="row">
                            <div class="form-group col-6">
                                ...
                                <input type="text" id="name_{{$language["code"]}}" name="name_{{$language["code"]}}" value="{{ old('name_' . $language["code"]) }}" class="form-control" placeholder="Ürün adını giriniz" />
                            </div>
                            <div class="form-group col-6">
                                <label for="base_{{$language["code"]}}">Ürün Tabanı ({{$language['name']}}) <span class="text-danger">*</span></label>
                                <input type="text" id="base_{{$language["code"]}}" name="base_{{$language["code"]}}" value="{{ old('base_' . $language["code"]) }}" class="form-control" placeholder="Ürün rengini giriniz" />
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-6">
                                <label for="color_{{$language["code"]}}">Ürün Rengi ({{$language['name']}}) <span class="text-danger">*</span></label>
                                <input type="text" id="color_{{$language["code"]}}" name="color_{{$language["code"]}}" value="{{ old('color_' . $language["code"]) }}" class="form-control" placeholder="Ürün rengini giriniz" />
                            </div>

                            <div class="form-group col-6">
                                <label for="material_{{$language["code"]}}">Ürün Materyali ({{$language['name']}}) <span class="text-danger">*</span></label>
                                <input type="text" id="material_{{$language["code"]}}" name="material_{{$language["code"]}}" value="{{ old('material_' . $language["code"]) }}" class="form-control" placeholder="Ürün rengini giriniz" />
                            </div>
                        </div>
                        <div class="form-group">
